...
import { index as countriesIndex, create as countriesCreate, edit as countriesEdit, destroy as countriesDestroy, toggleActive as countriesToggleActive } from '@/routes/countries';
...
export default function CountriesIndex({ countries, filters }: Props) {
    ...
    const pagination = useMemo(() => ({
        previous: countries.links.find((l) => l.label.includes('Previous')) ?? null,
        next:     countries.links.find((l) => l.label.includes('Next'))     ?? null,
        pages:    countries.links.filter((l) => /^\d+$/.test(cleanPaginationLabel(l.label))),
    }), [countries.links]);
    ...
    const applyFilters = () => {
        filterPopoverRef.current?.removeAttribute('open');
        router.get(countriesIndex.url(), form, { preserveState: true, preserveScroll: true, replace: true });
    };
    ...
    const clearFilters = () => {
        ...
        setForm(reset);
        filterPopoverRef.current?.removeAttribute('open');
        router.get(countriesIndex.url(), {}, { preserveState: true, preserveScroll: true, replace: true });
    };

    const updatePerPage = (nextValue: string) => {
        setForm((cur) => ({ ...cur, per_page: nextValue }));
        router.get(countriesIndex.url(), { ...form, per_page: nextValue, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    useDebouncedInertiaSearch({
        value: form.search,
        onSearch: (value, { onCancelToken }) => {
            router.get(countriesIndex.url(), { ...form, search: value, page: '1' }, { preserveState: true, preserveScroll: true, replace: true, onCancelToken });
        },
    });

    function confirmDelete(country: Country) {
        confirm(`Delete country "${country.name}"? This cannot be undone.`, () => router.delete(countriesDestroy.url(country.id)), { title: 'Delete Country', confirmLabel: 'Delete', variant: 'danger' });
    }

    function toggleActive(country: Country) {
        router.patch(countriesToggleActive.url(country.id), { is_active: !country.is_active });
    }
    ...
    return (
        <>
            ...
            <TableCard
                ...
                title="Countries"
                description="Browse and manage all countries."
                toolbar={
                    <>
                        <TableSearchInput
                            value={form.search}
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-4">
                            <p className="text-xs font-medium text-muted-foreground dark:text-stone-400">
                                Showing{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{countries.from ?? 0} - {countries.to ?? 0}</span>
                                {' '}of{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{countries.total}</span>
                                {' '}results
                            </p>
                    ...
                    <table className="w-full min-w-[480px] text-left">
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {countries.data.length === 0 && (
                                <tr>
                                    <td colSpan={4} className="px-6 py-12 text-center text-sm text-muted-foreground dark:text-stone-400">No countries found.</td>
                                </tr>
                            )}
                            {countries.data.map((country) => (
                                <tr key={country.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
                                                <span className="material-symbols-outlined text-[18px]">flag</span>
                                            </div>
                                            <span className="font-bold text-gray-900 dark:text-gray-100">{country.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 font-mono text-sm text-gray-600 dark:text-gray-400">{country.code ?? '-'}</td>
                                    <td className="px-6 py-4">
                                        <span className={cn('inline-flex rounded-full px-3 py-1 text-[11px] font-bold tracking-wider uppercase', country.is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-stone-100 text-stone-500 dark:bg-stone-800 dark:text-stone-400')}>
                                            {country.is_active ? 'Active' : 'Inactive'}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <ActionDropdown
                                            isOpen={openActionId === country.id}
                                            itemId={country.id}
                                            itemLabel={country.name}
                                            onToggle={(id) => setOpenActionId((cur) => (id === null ? null : cur === id ? null : id as number))}
                                            actions={[
                                                { id: `edit-${country.id}`, label: 'Edit country', icon: 'edit', href: countriesEdit.url(country.id) },
                                                {
                                                    id: `toggle-${country.id}`,
                                                    label: country.is_active ? 'Deactivate' : 'Activate',
                                                    icon: country.is_active ? 'toggle_off' : 'toggle_on',
