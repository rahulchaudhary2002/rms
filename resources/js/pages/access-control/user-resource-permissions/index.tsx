...
type Props = {
    resourcePerms: PaginatedResourcePerms;
    users: User[];
    permissions: Permission[];
    resourceTypes: string[];
    filters: { search?: string; user_id?: string; permission_id?: string; resource_type?: string; effect?: string; is_active?: string; per_page?: string };
};
...
export default function UserResourcePermissionsIndex({ resourcePerms, users, permissions, resourceTypes, filters }: Props) {
    ...
    const pagination = useMemo(() => ({
        previous: resourcePerms.links.find((l) => l.label.includes('Previous')) ?? null,
        next: resourcePerms.links.find((l) => l.label.includes('Next')) ?? null,
        pages: resourcePerms.links.filter((l) => /^\d+$/.test(cleanPaginationLabel(l.label))),
    }), [resourcePerms.links]);
    ...
    return (
        <>
            ...
            <TableCard
                ...
                description="Browse and manage all user resource-level permissions."
                toolbar={
                    <>
                        <TableSearchInput
                            value={form.search}
                        ...
                        <details ref={filterPopoverRef} className="relative">
                            ...
                            <div className="absolute right-0 z-50 mt-2 w-72 rounded-xl border border-border/20 bg-white p-5 shadow-2xl dark:border-border dark:bg-card">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-[10px] font-bold tracking-wider text-muted-foreground/60 uppercase">Resource Type</label>
                                        <SearchableSelect value={form.resource_type} onChange={(e) => setForm((c) => ({ ...c, resource_type: e.target.value }))}>
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-4">
                            <p className="text-xs font-medium text-muted-foreground dark:text-stone-400">
                                Showing{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{resourcePerms.from ?? 0} - {resourcePerms.to ?? 0}</span>
                                {' '}of{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{resourcePerms.total}</span>
                                {' '}results
                            </p>
                    ...
                    <table className="w-full min-w-[800px] text-left">
                        <thead>
                            <tr className="bg-muted text-[11px] font-bold tracking-[0.1em] text-muted-foreground uppercase dark:bg-stone-900 dark:text-stone-400">
                                ...
                                <th className="border-b border-border/10 px-6 py-4">Resource</th>
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            ...
                            {resourcePerms.data.map((rp) => (
                                <tr key={rp.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-semibold text-gray-900 dark:text-gray-100">{rp.user?.name}</div>
                                                <div className="text-xs text-muted-foreground dark:text-stone-400">{rp.user?.email}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 font-mono text-xs text-gray-600 dark:text-gray-300">{rp.permission?.slug}</td>
                                    <td className="px-6 py-4">
                                        <div className="font-medium text-sm text-gray-900 dark:text-gray-100">{rp.resource_type}</div>
                                        <div className="text-xs text-muted-foreground dark:text-stone-400">#{rp.resource_id}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={cn(
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <ActionDropdown
                                            isOpen={openActionId === rp.id}
                                            itemId={rp.id}
                                            itemLabel={`${rp.user?.name} → ${rp.resource_type}#${rp.resource_id}`}
                                            onToggle={(id) => toggleActionMenu(id as number | null)}
                                            actions={[
                                                {
                                                    id: `toggle-${rp.id}`,
                                                    label: rp.is_active ? 'Deactivate' : 'Activate',
                                                    icon: rp.is_active ? 'toggle_off' : 'toggle_on',
                                                    onClick: () => toggleActive(rp),
                                                },
                                                {
                                                    id: `delete-${rp.id}`,
