...
export default function PurchaseOrdersIndex({ orders, suppliers, warehouses, filters }: Props) {
    ...
    const pagination = useMemo(() => ({
        previous: orders.links.find((l) => l.label.includes('Previous')) ?? null,
        next:     orders.links.find((l) => l.label.includes('Next'))     ?? null,
        pages:    orders.links.filter((l) => /^\d+$/.test(cleanLabel(l.label))),
    }), [orders.links]);

    const hasActiveFilters = form.supplier_id !== '' || form.warehouse_id !== '' || form.status !== '';
    ...
    useDebouncedInertiaSearch({
        ...
        onSearch: (value, { onCancelToken }) => {
            router.get(ordersIndex.url(), { ...form, search: value, page: '1' }, { preserveState: true, preserveScroll: true, replace: true, onCancelToken });
        },
    });

    const applyFilters = () => {
        filterPopoverRef.current?.removeAttribute('open');
        router.get(ordersIndex.url(), { ...form, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };
    ...
    const clearFilters = () => {
        ...
        setForm(next);
        filterPopoverRef.current?.removeAttribute('open');
        router.get(ordersIndex.url(), { ...next, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    function handleDelete(order: PurchaseOrder) {
        confirm(
            `Are you sure you want to delete "${order.purchase_order_no}"?`,
            () => router.delete(ordersDestroy.url(order), { preserveScroll: true }),
            { title: 'Delete Purchase Order', confirmLabel: 'Delete', variant: 'danger' },
        );
    }

    function canReceive(order: PurchaseOrder): boolean {
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                description="Track and manage supplier purchase orders."
                actions={
            ...
            <TableCard
                    title="Purchase Orders"
                    description="Browse all purchase orders with supplier, warehouse, amount, and status."
                    toolbar={
                        <>
                        <TableSearchInput
                            value={form.search}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <SearchableSelect value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value }))}>
                                            ...
                                            {(Object.keys(STATUS_LABELS) as PurchaseOrderStatus[]).map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}
                                        </SearchableSelect>
                                    </div>
                                </div>
                        ...
                        <>
                            <div className="flex flex-wrap items-center gap-4">
                                <p className="text-xs font-medium text-muted-foreground dark:text-stone-400">
                                    Showing{' '}
                                    <span className="font-bold text-foreground dark:text-stone-100">{orders.from ?? 0} - {orders.to ?? 0}</span>
                                    {' '}of{' '}
                                    <span className="font-bold text-foreground dark:text-stone-100">{orders.total}</span>
                                    {' '}results
                                </p>
                                    ...
                                    <div className="relative">
                                        <select
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setForm((f) => ({ ...f, per_page: v }));
                                                router.get(ordersIndex.url(), { ...form, per_page: v, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
                                            }}
                            ...
                            <nav className="flex items-center gap-2" aria-label="Pagination">
                                <Link
                                    ...
                                    className={cn(
                                        ...
                                        pagination.previous?.url ? 'text-muted-foreground hover:bg-accent dark:text-stone-200 dark:hover:bg-stone-800' : 'pointer-events-none text-muted-foreground/40 dark:text-stone-600',
                                    )}
                                >
                    ...
                    <table className="w-full min-w-[800px] text-left text-sm">
                        <thead>
                            <tr className="bg-muted text-[11px] font-bold tracking-[0.1em] text-muted-foreground uppercase dark:bg-stone-900 dark:text-stone-400">
                                <th className="border-b border-border/10 px-6 py-4">Order No</th>
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {orders.data.length === 0 ? (
                                <tr><td colSpan={7} className="px-6 py-12 text-center text-muted-foreground">No purchase orders found.</td></tr>
                            ) : orders.data.map((order) => (
                                <tr key={order.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4 font-medium">
                                        <Link href={ordersShow.url(order)} className="hover:text-primary">{order.purchase_order_no}</Link>
                                    </td>
                                    <td className="px-6 py-4 text-muted-foreground">{order.supplier?.name ?? '—'}</td>
                                    <td className="px-6 py-4 text-muted-foreground">{order.warehouse?.name ?? '—'}</td>
                                    <td className="px-6 py-4 text-muted-foreground">{order.order_date}</td>
                                    <td className="px-6 py-4 text-right font-medium">{Number(order.grand_total).toFixed(2)}</td>
                                    <td className="px-6 py-4">
                                        <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset', STATUS_COLORS[order.status])}>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <ActionDropdown
                                            ...
                                            onOpenChange={(open) => setOpenActionId(open ? order.id : null)}
                                            items={[
                                                { label: 'View', href: ordersShow.url(order), icon: 'visibility', permission: 'purchase-orders-view' },
                                                ...(canReceive(order) ? [{ label: 'Receive', href: receivesCreate.url({ query: { purchase_order_id: order.id } }), icon: 'inventory', permission: 'purchase-receives-create' }] : []),
                                                ...(order.status === 'draft' ? [{ label: 'Edit', href: ordersEdit.url(order), icon: 'edit', permission: 'purchase-orders-edit' }] : []),
                                                ...(order.status === 'draft' ? [{ label: 'Delete', icon: 'delete', permission: 'purchase-orders-delete', onClick: () => handleDelete(order), variant: 'destructive' as const }] : []),
                                            ]}
                                        />
                                    </td>
                                </tr>
