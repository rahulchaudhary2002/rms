...
import {
    ...
    update as adjustmentsUpdate,
} from '@/routes/ingredient-stock-adjustments';
import { cn } from '@/lib/utils';
import type { Ingredient, IngredientStockAdjustment, Warehouse } from '@/types';
...
function emptyItem(): ItemRow {
    return { ingredient_id: '', ingredient_batch_id: '', actual_quantity: '', remarks: '' };
}

export default function IngredientStockAdjustmentsEdit({ adjustment, warehouses, ingredients, stockByIngredient }: Props) {
    const initialItems: ItemRow[] = (adjustment.items ?? []).map((i) => ({
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        post(adjustmentsUpdate.url(adjustment.id));
    }

    const ingredientById = Object.fromEntries(ingredients.map((i) => [String(i.id), i]));

    function getDifference(item: ItemRow): number | null {
        if (!item.ingredient_id || item.actual_quantity === '') return null;
        const sys = parseFloat(stockByIngredient[item.ingredient_id]?.quantity ?? '0');
    ...
    return (
        <>
            <Head title={`Edit ${adjustment.adjustment_no}`} />
            <PageHeader
                breadcrumbs={[
                    { label: 'Home', href: dashboard.url() },
                    { label: 'Stock Adjustments', href: adjustmentsIndex.url() },
                    { label: adjustment.adjustment_no, href: adjustmentsShow.url(adjustment.id) },
                    { label: 'Edit' },
                ]}
                title={`Edit ${adjustment.adjustment_no}`}
                description="Update draft stock adjustment."
            />

            <form onSubmit={submit} className="space-y-8 pb-6">
                <FormSection title="Adjustment Details" description="Update warehouse, date, and reason.">
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <FormField label="Adjustment Date" htmlFor="adjustment_date" error={errors.adjustment_date}>
                            <Input id="adjustment_date" type="date" value={data.adjustment_date} onChange={(e) => setData('adjustment_date', e.target.value)} />
                        </FormField>

                        <FormField label="Reason" htmlFor="reason" error={errors.reason} className="md:col-span-2">
                            <Input id="reason" value={data.reason} onChange={(e) => setData('reason', e.target.value)} placeholder="Describe why this adjustment is needed…" />
                        </FormField>
                    </div>
                </FormSection>

                <FormSection title="Items" description="Update actual quantities. Differences are recalculated on save.">
                    <div className="space-y-3">
                        {items.map((item, index) => {
                            const diff = getDifference(item);
                            const systemQty = item.ingredient_id ? parseFloat(stockByIngredient[item.ingredient_id]?.quantity ?? '0') : 0;
                            ...
                            return (
                                <div key={index} className="rounded-lg border border-border/30 bg-muted/30 p-4 dark:border-stone-700 dark:bg-stone-900/30">
                                    <div className="grid grid-cols-1 gap-3 md:grid-cols-[2fr_1fr_1fr_1fr_auto]">
                                        ...
                                        <FormField label={`Actual Qty${unit ? ` (${unit})` : ''}`} error={(errors as Record<string, string>)[`items.${index}.actual_quantity`]}>
                                            <Input
                                                type="number"
                                                min="0"
                                                step="0.0001"
                                                value={item.actual_quantity}
                                                onChange={(e) => updateItem(index, 'actual_quantity', e.target.value)}
                                                placeholder="0"
                ...
                <div className="flex flex-wrap items-center justify-end gap-4 border-t border-border/70 pt-8 dark:border-stone-700">
                    ...
                    <Link href={adjustmentsShow.url(adjustment.id)} className="rounded-lg px-6 py-2.5 text-sm font-semibold text-muted-foreground transition-colors hover:bg-secondary">
                        Discard
                    </Link>
                    <button
                        type="submit"
