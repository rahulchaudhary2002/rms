...
import {
    ...
    toggleStatus as customersToggleStatus,
...
export default function CustomersIndex({ customers, filters }: Props) {
    ...
    const pagination = useMemo(
        () => ({
            previous:
                customers.links.find((l) => l.label.includes('Previous')) ??
                null,
            next: customers.links.find((l) => l.label.includes('Next')) ?? null,
            pages: customers.links.filter((l) =>
                /^\d+$/.test(cleanPaginationLabel(l.label)),
            ),
        }),
        [customers.links],
    );
    ...
    const applyFilters = () => {
        filterPopoverRef.current?.removeAttribute('open');
        router.get(customersIndex.url(), form, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const clearFilters = () => {
        const reset = { search: '', status: 'all', per_page: '10' };
        setForm(reset);
        filterPopoverRef.current?.removeAttribute('open');
        router.get(
            customersIndex.url(),
    ...
    function toggleStatus(customer: Customer) {
        router.patch(customersToggleStatus.url(customer.id));
    }
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                title="Customers"
                description="Manage global customer profiles."
                actions={
            ...
            <TableCard
                ...
                title="Customers"
                description="Browse and manage all global customers."
                toolbar={
                    <>
                        <TableSearchInput
                            value={form.search}
                            onChange={(value) =>
                                setForm((cur) => ({ ...cur, search: value }))
                            }
                        ...
                        <details ref={filterPopoverRef} className="relative">
                            ...
                            <div className="absolute right-0 z-50 mt-2 w-72 rounded-xl border border-border/20 bg-white p-5 shadow-2xl dark:border-border dark:bg-card">
                                <div className="mb-4 flex items-center justify-between">
                                    <h4 className="text-sm font-bold text-foreground dark:text-stone-100">
                                        Table Filters
                                    </h4>
                                    <button
                                        type="button"
                                ...
                                <div className="space-y-4">
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-[10px] font-bold tracking-wider text-muted-foreground/60 uppercase">
                                            Status
                                        </label>
                                        <SearchableSelect
                                            value={form.status}
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {customers.data.length === 0 && (
                                <tr>
                                    <td
                                        colSpan={7}
                                        className="px-6 py-12 text-center text-sm text-muted-foreground dark:text-stone-400"
                                    >
                                        No customers found.
                                    </td>
                                </tr>
                            ...
                            {customers.data.map((customer) => (
                                <tr
                                    key={customer.id}
                                    className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50"
                                >
