...
import {
    ...
    toggleStatus as categoriesToggleStatus,
} from '@/routes/food-categories';
...
type Props = {
    ...
    parentCategories: Pick<FoodCategory, 'id' | 'name' | 'parent_id'>[];
...
export default function FoodCategoriesIndex({ categories, parentCategories, filters }: Props) {
    ...
    return (
        <>
            ...
            <PageHeader
                breadcrumbs={[{ label: 'Home', href: dashboard.url() }, { label: 'Food Categories' }]}
                title="Food Categories"
                description="Organise your menu into categories and sub-categories."
                actions={
                    <Can permission="food-categories-create">
                        <Link
            ...
            <TableCard
                ...
                title="Food Categories"
                description="Browse and manage all food categories."
                toolbar={
                    <>
                        <TableSearchInput
                            value={form.search}
                            ...
                            <div className="absolute right-0 z-50 mt-2 w-72 rounded-xl border border-border/20 bg-white p-5 shadow-2xl dark:border-border dark:bg-card">
                                ...
                                <div className="space-y-4">
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <SearchableSelect value={form.parent_id} onChange={(e) => setForm((f) => ({ ...f, parent_id: e.target.value }))}>
                    ...
                    <table className="w-full min-w-[700px] text-left">
                        <thead>
                            <tr className="bg-muted text-[11px] font-bold tracking-[0.1em] text-muted-foreground uppercase dark:bg-stone-900 dark:text-stone-400">
                                ...
                                <th className="border-b border-border/10 px-6 py-4">Foods</th>
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            ...
                            {categories.data.map((cat) => (
                                <tr key={cat.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
                                                <span className="material-symbols-outlined text-[18px]">category</span>
                                            </div>
                                            <Link href={categoriesEdit.url(cat.id)} className="font-bold text-gray-900 transition-colors hover:text-primary dark:text-gray-100">
                                                {cat.name}
                                            </Link>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-sm text-muted-foreground dark:text-stone-400">{cat.foods_count ?? 0}</td>
                                    <td className="px-6 py-4 text-sm text-muted-foreground dark:text-stone-400">{cat.sort_order}</td>
                                    <td className="px-6 py-4"><StatusBadge active={cat.is_active} /></td>
                                    <td className="px-6 py-4 text-right">
                                        <ActionDropdown
                                            isOpen={openActionId === cat.id}
                                            itemId={cat.id}
                                            itemLabel={cat.name}
                                            onToggle={(id) =>
                                                setOpenActionId((cur) =>
