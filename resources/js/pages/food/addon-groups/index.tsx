...
import {
    ...
    toggleStatus as groupsToggleStatus,
} from '@/routes/addon-groups';
...
export default function AddonGroupsIndex({ addonGroups: groups, filters }: Props) {
    ...
    const pagination = useMemo(
        () => ({
            previous: groups.links.find((l) => l.label.includes('Previous')) ?? null,
            next: groups.links.find((l) => l.label.includes('Next')) ?? null,
            pages: groups.links.filter((l) => /^\d+$/.test(cleanPaginationLabel(l.label))),
        }),
        [groups.links],
    );
    ...
    const applyFilters = () => {
        filterPopoverRef.current?.removeAttribute('open');
        router.get(groupsIndex.url(), form, { preserveState: true, preserveScroll: true, replace: true });
    };
    ...
    const clearFilters = () => {
        ...
        setForm(reset);
        filterPopoverRef.current?.removeAttribute('open');
        router.get(groupsIndex.url(), {}, { preserveState: true, preserveScroll: true, replace: true });
    };
    ...
    return (
        <>
            ...
            <PageHeader
                breadcrumbs={[{ label: 'Home', href: dashboard.url() }, { label: 'Add-on Groups' }]}
                title="Add-on Groups"
                description="Manage add-on groups and their add-ons for menu customisation."
                actions={
                    <Can permission="addon-groups-create">
                        <Link
            ...
            <TableCard
                ...
                title="Add-on Groups"
                description="Browse and manage all add-on groups."
                toolbar={
                    <>
                        <TableSearchInput
                            value={form.search}
                    ...
                    <table className="w-full min-w-[700px] text-left">
                        <thead>
                            <tr className="bg-muted text-[11px] font-bold tracking-[0.1em] text-muted-foreground uppercase dark:bg-stone-900 dark:text-stone-400">
                                ...
                                <th className="border-b border-border/10 px-6 py-4">Required</th>
                                <th className="border-b border-border/10 px-6 py-4">Min</th>
                                <th className="border-b border-border/10 px-6 py-4">Max</th>
                                <th className="border-b border-border/10 px-6 py-4">Add-ons</th>
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            ...
                            {groups.data.map((group) => (
                                <tr key={group.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
                                                <span className="material-symbols-outlined text-[18px]">add_circle</span>
                                            </div>
                                            <Link href={groupShowUrl(group.id)} className="font-bold text-gray-900 transition-colors hover:text-primary dark:text-gray-100">
                                                {group.name}
                                            </Link>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-sm text-muted-foreground dark:text-stone-400">{group.min_select}</td>
                                    <td className="px-6 py-4 text-sm text-muted-foreground dark:text-stone-400">{group.max_select ?? '-'}</td>
                                    <td className="px-6 py-4 text-sm text-muted-foreground dark:text-stone-400">{group.addons_count ?? 0}</td>
                                    <td className="px-6 py-4"><StatusBadge active={group.is_active} /></td>
                                    <td className="px-6 py-4 text-right">
                                        <ActionDropdown
                                            isOpen={openActionId === group.id}
                                            itemId={group.id}
                                            ...
                                            actions={[
                                                { id: `view-${group.id}`, label: 'View group', icon: 'visibility', href: groupShowUrl(group.id) },
                                                { id: `edit-${group.id}`, label: 'Edit group', icon: 'edit', href: groupsEdit.url(group.id) },
                                                { id: `toggle-${group.id}`, label: group.is_active ? 'Deactivate' : 'Activate', icon: group.is_active ? 'toggle_off' : 'toggle_on', onClick: () => router.patch(groupsToggleStatus.url(group.id)) },
                                                {
                                                    id: `delete-${group.id}`,
                                                    label: 'Delete group',
                                                    icon: 'delete',
