...
export default function FoodRecipesIndex({ recipes, filters }: Props) {
    ...
    return (
        <>
            <Head title="Food Recipes" />
            <PageHeader
                breadcrumbs={[{ label: 'Home', href: dashboard.url() }, { label: 'Recipe' }, { label: 'Food Recipes' }]}
                title="Food Recipes"
            ...
            <TableCard
                ...
                title="Food Recipes"
                description="Recipe lines used for stock deduction and food preparation."
                toolbar={
                    <>
                        <TableSearchInput
                            value={form.search}
                            onChange={(value) => setForm((cur) => ({ ...cur, search: value }))}
                            placeholder="Search food, variant, ingredient..."
                    ...
                    <table className="w-full min-w-[760px] text-left">
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {recipes.data.length === 0 && (
                                <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-muted-foreground">No food recipe lines found.</td></tr>
                            )}
                            {recipes.data.map((recipe) => (
                                <tr key={recipe.id} className="transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4 font-bold text-foreground">{recipe.food?.name ?? '-'}</td>
                                    <td className="px-6 py-4 text-sm text-muted-foreground">{recipe.variant?.name ?? 'Base recipe'}</td>
                                        ...
                                        <ActionDropdown
                                            ...
                                            actions={[
                                                { id: `edit-${recipe.id}`, label: 'Edit recipe', icon: 'edit', href: recipesEdit.url(recipe.id) },
                                                {
                                                    ...
                                                    onClick: () => confirm('Delete this food recipe line?', () => router.delete(recipesDestroy.url(recipe.id)), { title: 'Delete Recipe', confirmLabel: 'Delete', variant: 'danger' }),
                                                },
                                            ]}
                                        />
                                    </td>
