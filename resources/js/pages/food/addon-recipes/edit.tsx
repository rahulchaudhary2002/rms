...
import type { Addon, AddonGroup, AddonRecipe, Ingredient, Unit } from '@/types';
...
const indexUrl = '/recipes/addons';
const updateUrl = (id: number) => `/recipes/addons/${id}`;

export default function AddonRecipeEdit({ recipe, addons, ingredients, units }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        addon_id: String(recipe.addon_id),
        ingredient_id: String(recipe.ingredient_id),
        unit_id: String(recipe.unit_id),
        quantity: String(recipe.quantity),
        wastage_quantity: String(recipe.wastage_quantity ?? '0'),
    ...
    return (
        <>
            <Head title="Edit Add-on Recipe" />
            <PageHeader
                breadcrumbs={[
                    { label: 'Home', href: dashboard.url() },
                    { label: 'Add-on Recipes', href: indexUrl },
                    { label: 'Edit' },
                ]}
                title="Edit Add-on Recipe"
                description={recipe.addon?.name ?? 'Update this add-on recipe line.'}
            />

            <form onSubmit={submit} className="space-y-8 pb-6">
                <FormSection title="Recipe Line" description="Update the add-on, ingredient, usage unit, and quantity.">
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <FormField label="Quantity" htmlFor="quantity" error={errors.quantity}>
                            <Input id="quantity" type="number" min="0" step="0.0001" value={data.quantity} onChange={(e) => setData('quantity', e.target.value)} />
                        </FormField>

                        <FormField label="Wastage Quantity" htmlFor="wastage_quantity" error={errors.wastage_quantity}>
                            <Input id="wastage_quantity" type="number" min="0" step="0.0001" value={data.wastage_quantity} onChange={(e) => setData('wastage_quantity', e.target.value)} />
                        </FormField>
