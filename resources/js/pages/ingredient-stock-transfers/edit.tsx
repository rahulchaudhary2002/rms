...
import {
    ...
    update as transfersUpdate,
} from '@/routes/ingredient-stock-transfers';
import type { Ingredient, IngredientStockTransfer, Warehouse } from '@/types';
...
export default function IngredientStockTransfersEdit({ transfer, warehouses, ingredients }: Props) {
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        post(transfersUpdate.url(transfer.id));
    }
    ...
    return (
        <>
            <Head title={`Edit ${transfer.transfer_no}`} />
            <PageHeader
                breadcrumbs={[
                    { label: 'Home', href: dashboard.url() },
                    { label: 'Stock Transfers', href: transfersIndex.url() },
                    { label: transfer.transfer_no, href: transfersShow.url(transfer.id) },
                    { label: 'Edit' },
                ]}
                title={`Edit ${transfer.transfer_no}`}
                description="Update draft transfer details."
            />

            <form onSubmit={submit} className="space-y-8 pb-6">
                <FormSection title="Transfer Details" description="Update source and destination warehouses and the transfer date.">
                ...
                <FormSection title="Items" description="Update ingredients and quantities. All quantities must be in the ingredient's base unit.">
                    <div className="space-y-3">
                        {items.map((item, index) => (
                            <div key={index} className="grid grid-cols-1 gap-3 rounded-lg border border-border/30 bg-muted/30 p-4 md:grid-cols-[1fr_1fr_auto_1fr_auto] dark:border-stone-700 dark:bg-stone-900/30">
                                ...
                                <FormField label="Remarks" error={(errors as Record<string, string>)[`items.${index}.remarks`]} className="md:col-span-2">
                ...
                <div className="flex flex-wrap items-center justify-end gap-4 border-t border-border/70 pt-8 dark:border-stone-700">
                    ...
                    <Link href={transfersShow.url(transfer.id)} className="rounded-lg px-6 py-2.5 text-sm font-semibold text-muted-foreground transition-colors hover:bg-secondary">
                        Discard
                    </Link>
                    <button
                        type="submit"
