...
import type { IngredientStockTransfer, TransferStatus, Warehouse } from '@/types';
...
type Props = {
    transfers: Paginated<IngredientStockTransfer>;
    warehouses: Pick<Warehouse, 'id' | 'name'>[];
    filters: { search?: string; from_warehouse_id?: string; to_warehouse_id?: string; status?: string; per_page?: string };
};
...
export default function IngredientStockTransfersIndex({ transfers, warehouses, filters }: Props) {
    ...
    const pagination = useMemo(() => ({
        previous: transfers.links.find((l) => l.label.includes('Previous')) ?? null,
        next:     transfers.links.find((l) => l.label.includes('Next'))     ?? null,
        pages:    transfers.links.filter((l) => /^\d+$/.test(cleanLabel(l.label))),
    }), [transfers.links]);

    const hasActiveFilters = form.from_warehouse_id !== '' || form.to_warehouse_id !== '' || form.status !== '';
    ...
    const applyFilters = (next: typeof form) => {
        router.get(transfersIndex.url(), { ...next, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    const updatePerPage = (nextValue: string) => {
        setForm((cur) => ({ ...cur, per_page: nextValue }));
        router.get(transfersIndex.url(), { ...form, per_page: nextValue, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    useDebouncedInertiaSearch({
        value: form.search,
        onSearch: (value, { onCancelToken }) => {
            router.get(transfersIndex.url(), { ...form, search: value, page: '1' }, { preserveState: true, preserveScroll: true, replace: true, onCancelToken });
        },
    });
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                description="Transfer ingredients between warehouses."
                actions={
                    <Can permission="ingredient-stock-transfers-create">
                        <Link
            ...
            <TableCard
                title="Stock Transfers"
                description="Browse and manage all ingredient stock transfers."
                toolbar={
                        ...
                        <details ref={filterPopoverRef} className="relative">
                            ...
                            <div className="absolute right-0 z-20 mt-2 w-64 rounded-xl border border-border/20 bg-white p-4 shadow-xl dark:border-stone-700 dark:bg-stone-900">
                                <div className="space-y-3">
                                    <div>
                                        <label className="mb-1 block text-[11px] font-bold uppercase tracking-wider text-muted-foreground">From Warehouse</label>
                                        <SearchableSelect value={form.from_warehouse_id} onChange={(e) => { const next = { ...form, from_warehouse_id: e.target.value }; setForm(next); applyFilters(next); }}>
                                            <option value="">All</option>
                                            {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
                                        </SearchableSelect>
                                    </div>
                                    <div>
                                        <label className="mb-1 block text-[11px] font-bold uppercase tracking-wider text-muted-foreground">To Warehouse</label>
                                        <SearchableSelect value={form.to_warehouse_id} onChange={(e) => { const next = { ...form, to_warehouse_id: e.target.value }; setForm(next); applyFilters(next); }}>
                                            <option value="">All</option>
                                            {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
                                        </SearchableSelect>
                                    </div>
                                    <div>
                                        ...
                                        <SearchableSelect value={form.status} onChange={(e) => { const next = { ...form, status: e.target.value }; setForm(next); applyFilters(next); }}>
                                            <option value="">All</option>
                                            {(Object.keys(STATUS_LABELS) as TransferStatus[]).map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}
                                        </SearchableSelect>
                                    </div>
                                    {hasActiveFilters && (
                                        <Button variant="ghost" size="sm" className="w-full text-xs" onClick={() => { const next = { ...form, from_warehouse_id: '', to_warehouse_id: '', status: '' }; setForm(next); applyFilters(next); }}>
                                            Clear filters
                                        </Button>
                                    )}
                                </div>
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-4">
                            <p className="text-xs font-medium text-muted-foreground dark:text-stone-400">
                                Showing{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{transfers.from ?? 0} - {transfers.to ?? 0}</span>
                                {' '}of{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{transfers.total}</span>
                                {' '}results
                            </p>
                    ...
                    <table className="w-full min-w-[750px] text-left">
                        <thead>
                            <tr className="bg-muted text-[11px] font-bold tracking-[0.1em] text-muted-foreground uppercase dark:bg-stone-900 dark:text-stone-400">
                                <th className="border-b border-border/10 px-6 py-4">Transfer No</th>
                                <th className="border-b border-border/10 px-6 py-4">Date</th>
                                <th className="border-b border-border/10 px-6 py-4">From</th>
                                <th className="border-b border-border/10 px-6 py-4">To</th>
                                <th className="border-b border-border/10 px-6 py-4">Status</th>
                                <th className="border-b border-border/10 px-6 py-4">Requested By</th>
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {transfers.data.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="px-6 py-12 text-center text-sm text-muted-foreground dark:text-stone-400">No stock transfers found.</td>
                                </tr>
                            )}
                            {transfers.data.map((transfer) => (
                                <tr key={transfer.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4">
                                        <Link href={transfersShow.url(transfer.id)} className="font-mono font-bold text-primary transition-colors hover:underline">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={cn('inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-bold tracking-wide ring-1', STATUS_COLORS[transfer.status])}>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <Link
                                            href={transfersShow.url(transfer.id)}
