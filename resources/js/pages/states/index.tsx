...
import { index as statesIndex, create as statesCreate, edit as statesEdit, destroy as statesDestroy, toggleActive as statesToggleActive } from '@/routes/states';
...
type Props = {
    ...
    filters: { search?: string; country_id?: string; is_active?: string; per_page?: string };
};
...
export default function StatesIndex({ states, countries, filters }: Props) {
    ...
    const pagination = useMemo(() => ({
        previous: states.links.find((l) => l.label.includes('Previous')) ?? null,
        next:     states.links.find((l) => l.label.includes('Next'))     ?? null,
        pages:    states.links.filter((l) => /^\d+$/.test(cleanPaginationLabel(l.label))),
    }), [states.links]);
    ...
    const applyFilters = () => {
        filterPopoverRef.current?.removeAttribute('open');
        router.get(statesIndex.url(), form, { preserveState: true, preserveScroll: true, replace: true });
    };

    const clearFilters = () => {
        const reset = { search: '', country_id: '', is_active: '', per_page: '10' };
        setForm(reset);
        filterPopoverRef.current?.removeAttribute('open');
        router.get(statesIndex.url(), {}, { preserveState: true, preserveScroll: true, replace: true });
    };

    const updatePerPage = (nextValue: string) => {
        setForm((cur) => ({ ...cur, per_page: nextValue }));
        router.get(statesIndex.url(), { ...form, per_page: nextValue, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    useDebouncedInertiaSearch({
        value: form.search,
        onSearch: (value, { onCancelToken }) => {
            router.get(statesIndex.url(), { ...form, search: value, page: '1' }, { preserveState: true, preserveScroll: true, replace: true, onCancelToken });
        },
    });

    function confirmDelete(state: State) {
        confirm(`Delete state "${state.name}"? This cannot be undone.`, () => router.delete(statesDestroy.url(state.id)), { title: 'Delete State', confirmLabel: 'Delete', variant: 'danger' });
    }

    function toggleActive(state: State) {
        router.patch(statesToggleActive.url(state.id), { is_active: !state.is_active });
    }
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                title="States"
                description="Manage states and provinces available in the system."
                actions={
                    <Can permission="states-create">
                        <Link
                            href={statesCreate.url()}
            ...
            <TableCard
                ...
                title="States"
                description="Browse and manage all states."
                toolbar={
                    <>
                        <TableSearchInput
                            value={form.search}
                            ...
                            <div className="absolute right-0 z-50 mt-2 w-72 rounded-xl border border-border/20 bg-white p-5 shadow-2xl dark:border-border dark:bg-card">
                                ...
                                <div className="space-y-4">
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <SearchableSelect value={form.country_id} onChange={(e) => setForm((cur) => ({ ...cur, country_id: e.target.value }))}>
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-4">
                            <p className="text-xs font-medium text-muted-foreground dark:text-stone-400">
                                Showing{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{states.from ?? 0} - {states.to ?? 0}</span>
                                {' '}of{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{states.total}</span>
                                {' '}results
                            </p>
                    ...
                    <table className="w-full min-w-[560px] text-left">
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {states.data.length === 0 && (
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-sm text-muted-foreground dark:text-stone-400">No states found.</td>
                                </tr>
                            )}
                            {states.data.map((state) => (
                                <tr key={state.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <ActionDropdown
                                            isOpen={openActionId === state.id}
                                            itemId={state.id}
                                            itemLabel={state.name}
                                            onToggle={(id) => setOpenActionId((cur) => (id === null ? null : cur === id ? null : id as number))}
                                            actions={[
                                                { id: `edit-${state.id}`, label: 'Edit state', icon: 'edit', href: statesEdit.url(state.id) },
                                                {
                                                    id: `toggle-${state.id}`,
                                                    label: state.is_active ? 'Deactivate' : 'Activate',
                                                    icon: state.is_active ? 'toggle_off' : 'toggle_on',
