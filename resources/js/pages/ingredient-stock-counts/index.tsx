...
import type { IngredientStockCount, StockCountStatus, Warehouse } from '@/types';
...
type Props = {
    counts: Paginated<IngredientStockCount>;
...
export default function IngredientStockCountsIndex({ counts, warehouses, filters }: Props) {
    ...
    const pagination = useMemo(() => ({
        previous: counts.links.find((l) => l.label.includes('Previous')) ?? null,
        next:     counts.links.find((l) => l.label.includes('Next'))     ?? null,
        pages:    counts.links.filter((l) => /^\d+$/.test(cleanLabel(l.label))),
    }), [counts.links]);
    ...
    const applyFilters = (next: typeof form) => {
        router.get(countsIndex.url(), { ...next, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    const updatePerPage = (nextValue: string) => {
        setForm((cur) => ({ ...cur, per_page: nextValue }));
        router.get(countsIndex.url(), { ...form, per_page: nextValue, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    useDebouncedInertiaSearch({
        value: form.search,
        onSearch: (value, { onCancelToken }) => {
            router.get(countsIndex.url(), { ...form, search: value, page: '1' }, { preserveState: true, preserveScroll: true, replace: true, onCancelToken });
        },
    });
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                title="Stock Counts"
                description="Perform physical inventory counts and reconcile stock differences."
                actions={
                    <Can permission="ingredient-stock-counts-create">
                        <Link
                            href={countsCreate.url()}
            ...
            <TableCard
                title="Stock Counts"
                description="Browse and manage all physical stock count records."
                toolbar={
                    <div className="flex flex-wrap items-center gap-3">
                        <TableSearchInput
                            value={form.search}
                            onChange={(value) => setForm((cur) => ({ ...cur, search: value }))}
                            placeholder="Search by count no…"
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <SearchableSelect value={form.status} onChange={(e) => { const next = { ...form, status: e.target.value }; setForm(next); applyFilters(next); }}>
                                            <option value="">All</option>
                                            {(Object.keys(STATUS_LABELS) as StockCountStatus[]).map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}
                                        </SearchableSelect>
                                    </div>
                                    {hasActiveFilters && (
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-4">
                            <p className="text-xs font-medium text-muted-foreground dark:text-stone-400">
                                Showing{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{counts.from ?? 0} - {counts.to ?? 0}</span>
                                {' '}of{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{counts.total}</span>
                                {' '}results
                            </p>
                    ...
                    <table className="w-full min-w-[620px] text-left">
                        <thead>
                            <tr className="bg-muted text-[11px] font-bold tracking-[0.1em] text-muted-foreground uppercase dark:bg-stone-900 dark:text-stone-400">
                                <th className="border-b border-border/10 px-6 py-4">Count No</th>
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {counts.data.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-sm text-muted-foreground dark:text-stone-400">No stock count records found.</td>
                                </tr>
                            )}
                            {counts.data.map((cnt) => (
                                <tr key={cnt.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4">
                                        <Link href={countsShow.url(cnt.id)} className="font-mono font-bold text-primary transition-colors hover:underline">
                                            {cnt.count_no}
                                        </Link>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-muted-foreground dark:text-stone-400">
                                        {new Date(cnt.count_date).toLocaleDateString()}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-foreground dark:text-stone-200">
                                        {cnt.warehouse?.name ?? '-'}
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={cn('inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-bold tracking-wide ring-1', STATUS_COLORS[cnt.status])}>
