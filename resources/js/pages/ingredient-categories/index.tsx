...
import { index as categoriesIndex, create as categoriesCreate, edit as categoriesEdit, destroy as categoriesDestroy, toggleActive as categoriesToggleActive } from '@/routes/ingredient-categories';
...
type Props = {
    ...
    parents: Pick<IngredientCategory, 'id' | 'name'>[];
    filters: { search?: string; parent_id?: string; is_active?: string; per_page?: string };
};
...
export default function IngredientCategoriesIndex({ categories, parents, filters }: Props) {
    ...
    const pagination = useMemo(() => ({
        previous: categories.links.find((l) => l.label.includes('Previous')) ?? null,
        next:     categories.links.find((l) => l.label.includes('Next'))     ?? null,
        pages:    categories.links.filter((l) => /^\d+$/.test(cleanPaginationLabel(l.label))),
    }), [categories.links]);
    ...
    const applyFilters = () => {
        filterPopoverRef.current?.removeAttribute('open');
        router.get(categoriesIndex.url(), form, { preserveState: true, preserveScroll: true, replace: true });
    };

    const clearFilters = () => {
        const reset = { search: '', parent_id: '', is_active: '', per_page: '10' };
        setForm(reset);
        filterPopoverRef.current?.removeAttribute('open');
        router.get(categoriesIndex.url(), {}, { preserveState: true, preserveScroll: true, replace: true });
    };

    const updatePerPage = (nextValue: string) => {
        setForm((cur) => ({ ...cur, per_page: nextValue }));
        router.get(categoriesIndex.url(), { ...form, per_page: nextValue, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    useDebouncedInertiaSearch({
        value: form.search,
        onSearch: (value, { onCancelToken }) => {
            router.get(categoriesIndex.url(), { ...form, search: value, page: '1' }, { preserveState: true, preserveScroll: true, replace: true, onCancelToken });
        },
    });

    function confirmDelete(category: IngredientCategory) {
        confirm(`Delete category "${category.name}"? This cannot be undone.`, () => router.delete(categoriesDestroy.url(category.id)), { title: 'Delete Category', confirmLabel: 'Delete', variant: 'danger' });
    }

    function toggleActive(category: IngredientCategory) {
        router.patch(categoriesToggleActive.url(category.id), { is_active: !category.is_active });
    }
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                description="Manage categories for organising ingredients."
                actions={
                    <Can permission="ingredient-categories-create">
                        <Link
            ...
            <TableCard
                ...
                title="Ingredient Categories"
                description="Browse and manage all ingredient categories."
                toolbar={
                    <>
                        <TableSearchInput
                            value={form.search}
                            ...
                            <div className="absolute right-0 z-50 mt-2 w-72 rounded-xl border border-border/20 bg-white p-5 shadow-2xl dark:border-border dark:bg-card">
                                ...
                                <div className="space-y-4">
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-[10px] font-bold tracking-wider text-muted-foreground/60 uppercase">Parent Category</label>
                                        <SearchableSelect value={form.parent_id} onChange={(e) => setForm((cur) => ({ ...cur, parent_id: e.target.value }))}>
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-4">
                            <p className="text-xs font-medium text-muted-foreground dark:text-stone-400">
                                Showing{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{categories.from ?? 0} - {categories.to ?? 0}</span>
                                {' '}of{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{categories.total}</span>
                                {' '}results
                            </p>
                    ...
                    <table className="w-full min-w-[560px] text-left">
                        <thead>
                            <tr className="bg-muted text-[11px] font-bold tracking-[0.1em] text-muted-foreground uppercase dark:bg-stone-900 dark:text-stone-400">
                                ...
                                <th className="border-b border-border/10 px-6 py-4">Parent</th>
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {categories.data.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-sm text-muted-foreground dark:text-stone-400">No ingredient categories found.</td>
                                </tr>
                            )}
                            {categories.data.map((category) => (
                                <tr key={category.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
                                                <span className="material-symbols-outlined text-[18px]">category</span>
                                            </div>
                                            <span className="font-bold text-gray-900 dark:text-gray-100">{category.name}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <ActionDropdown
                                            isOpen={openActionId === category.id}
                                            itemId={category.id}
                                            itemLabel={category.name}
                                            onToggle={(id) => setOpenActionId((cur) => (id === null ? null : cur === id ? null : id as number))}
                                            actions={[
                                                { id: `edit-${category.id}`, label: 'Edit category', icon: 'edit', href: categoriesEdit.url(category.id) },
                                                {
                                                    id: `toggle-${category.id}`,
                                                    label: category.is_active ? 'Deactivate' : 'Activate',
                                                    icon: category.is_active ? 'toggle_off' : 'toggle_on',
