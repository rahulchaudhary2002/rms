...
import {
    ...
    update as wastagesUpdate,
} from '@/routes/ingredient-wastages';
import type { Ingredient, IngredientWastage, WastageReason, Warehouse } from '@/types';
...
export default function IngredientWastagesEdit({ wastage, warehouses, ingredients }: Props) {
    const initialItems: ItemRow[] = (wastage.items ?? []).map((i) => ({
    ...
    const { data, setData, post, processing, errors } = useForm({
        _method:      'PUT',
        warehouse_id: String(wastage.warehouse_id),
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        post(wastagesUpdate.url(wastage.id));
    }
    ...
    return (
        <>
            <Head title={`Edit ${wastage.wastage_no}`} />
            <PageHeader
                breadcrumbs={[
                    { label: 'Home', href: dashboard.url() },
                    { label: 'Wastages', href: wastagesIndex.url() },
                    { label: wastage.wastage_no, href: wastagesShow.url(wastage.id) },
                    { label: 'Edit' },
                ]}
                title={`Edit ${wastage.wastage_no}`}
                description="Update draft wastage record."
            />

            <form onSubmit={submit} className="space-y-8 pb-6">
                <FormSection title="Wastage Details" description="Update warehouse, date, and reason.">
                ...
                <FormSection title="Items" description="Update ingredients and wasted quantities. All quantities must be in the ingredient's base unit.">
                ...
                <div className="flex flex-wrap items-center justify-end gap-4 border-t border-border/70 pt-8 dark:border-stone-700">
                    ...
                    <Link href={wastagesShow.url(wastage.id)} className="rounded-lg px-6 py-2.5 text-sm font-semibold text-muted-foreground transition-colors hover:bg-secondary">
                        Discard
                    </Link>
                    <button
                        type="submit"
