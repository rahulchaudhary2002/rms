...
import type { IngredientWastage, WastageReason, WastageStatus, Warehouse } from '@/types';
...
type Props = {
    wastages: Paginated<IngredientWastage>;
    warehouses: Pick<Warehouse, 'id' | 'name'>[];
    filters: { search?: string; warehouse_id?: string; status?: string; reason?: string; per_page?: string };
};
...
export default function IngredientWastagesIndex({ wastages, warehouses, filters }: Props) {
    ...
    const pagination = useMemo(() => ({
        previous: wastages.links.find((l) => l.label.includes('Previous')) ?? null,
        next:     wastages.links.find((l) => l.label.includes('Next'))     ?? null,
        pages:    wastages.links.filter((l) => /^\d+$/.test(cleanLabel(l.label))),
    }), [wastages.links]);

    const hasActiveFilters = form.warehouse_id !== '' || form.status !== '' || form.reason !== '';
    ...
    const applyFilters = (next: typeof form) => {
        router.get(wastagesIndex.url(), { ...next, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    const updatePerPage = (nextValue: string) => {
        setForm((cur) => ({ ...cur, per_page: nextValue }));
        router.get(wastagesIndex.url(), { ...form, per_page: nextValue, page: '1' }, { preserveState: true, preserveScroll: true, replace: true });
    };

    useDebouncedInertiaSearch({
        value: form.search,
        onSearch: (value, { onCancelToken }) => {
            router.get(wastagesIndex.url(), { ...form, search: value, page: '1' }, { preserveState: true, preserveScroll: true, replace: true, onCancelToken });
        },
    });

    return (
        <>
            <Head title="Ingredient Wastages" />
            <PageHeader
                breadcrumbs={[
                    { label: 'Home', href: dashboard.url() },
                    { label: 'Wastages' },
                ]}
                title="Ingredient Wastages"
                description="Record and track ingredient wastage."
                actions={
                    <Can permission="ingredient-wastages-create">
                        <Link
                            href={wastagesCreate.url()}
            ...
            <TableCard
                title="Wastages"
                description="Browse and manage all ingredient wastage records."
                toolbar={
                        ...
                        <details ref={filterPopoverRef} className="relative">
                            ...
                            <div className="absolute right-0 z-20 mt-2 w-64 rounded-xl border border-border/20 bg-white p-4 shadow-xl dark:border-stone-700 dark:bg-stone-900">
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        <label className="mb-1 block text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Reason</label>
                                        <SearchableSelect value={form.reason} onChange={(e) => { const next = { ...form, reason: e.target.value }; setForm(next); applyFilters(next); }}>
                                            <option value="">All</option>
                                            {(Object.keys(REASON_LABELS) as WastageReason[]).map((r) => <option key={r} value={r}>{REASON_LABELS[r]}</option>)}
                                        </SearchableSelect>
                                    </div>
                                    <div>
                                        ...
                                        <SearchableSelect value={form.status} onChange={(e) => { const next = { ...form, status: e.target.value }; setForm(next); applyFilters(next); }}>
                                            <option value="">All</option>
                                            {(Object.keys(STATUS_LABELS) as WastageStatus[]).map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}
                                        </SearchableSelect>
                                    </div>
                                    {hasActiveFilters && (
                                        <Button variant="ghost" size="sm" className="w-full text-xs" onClick={() => { const next = { ...form, warehouse_id: '', status: '', reason: '' }; setForm(next); applyFilters(next); }}>
                                            Clear filters
                                        </Button>
                                    )}
                                </div>
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-4">
                            <p className="text-xs font-medium text-muted-foreground dark:text-stone-400">
                                Showing{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{wastages.from ?? 0} - {wastages.to ?? 0}</span>
                                {' '}of{' '}
                                <span className="font-bold text-foreground dark:text-stone-100">{wastages.total}</span>
                                {' '}results
                            </p>
                    ...
                    <table className="w-full min-w-[650px] text-left">
                        <thead>
                            <tr className="bg-muted text-[11px] font-bold tracking-[0.1em] text-muted-foreground uppercase dark:bg-stone-900 dark:text-stone-400">
                                <th className="border-b border-border/10 px-6 py-4">Wastage No</th>
                                <th className="border-b border-border/10 px-6 py-4">Date</th>
                                <th className="border-b border-border/10 px-6 py-4">Warehouse</th>
                                <th className="border-b border-border/10 px-6 py-4">Reason</th>
                        ...
                        <tbody className="divide-y divide-muted dark:divide-stone-800">
                            {wastages.data.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="px-6 py-12 text-center text-sm text-muted-foreground dark:text-stone-400">No wastage records found.</td>
                                </tr>
                            )}
                            {wastages.data.map((wastage) => (
                                <tr key={wastage.id} className="group transition-colors hover:bg-muted dark:hover:bg-stone-900/50">
                                    <td className="px-6 py-4">
                                        <Link href={wastagesShow.url(wastage.id)} className="font-mono font-bold text-primary transition-colors hover:underline">
                                            {wastage.wastage_no}
                                        </Link>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-muted-foreground dark:text-stone-400">
                                        {new Date(wastage.wastage_date).toLocaleDateString()}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-foreground dark:text-stone-200">
                                        {wastage.warehouse?.name ?? '-'}
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="inline-flex rounded-full bg-slate-100 px-2.5 py-0.5 text-[11px] font-bold tracking-wide ring-1 ring-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:ring-slate-700">
                                            {REASON_LABELS[wastage.reason]}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={cn('inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-bold tracking-wide ring-1', STATUS_COLORS[wastage.status])}>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <Link
                                            href={wastagesShow.url(wastage.id)}
